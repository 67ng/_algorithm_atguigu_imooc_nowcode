...
/**
 * Description:有一排正数，玩家A和玩家B都可以看到。
...
public class CardsInLine {
    ...
    public static int f(int[] arr, int i, int j) {//在数组i,j范围里，作为先取者得到的最大分数
        if (i == j) {
            return arr[i];
        }
    ...
    public static int s(int[] arr, int i, int j) {//在数组i,j范围里，作为后取者得到的最大分数
        if (i == j) {
            return 0;
        }
    ...
    public static int win2(int[] arr) {
        ...
        //记忆化搜索
        int[][] f = new int[arr.length][arr.length];//先取者
        int[][] s = new int[arr.length][arr.length];//后取者
        for (int j = 0; j < arr.length; j++) {//for里的过程就是填满两个二维数组(右上半边)的过程
            f[j][j] = arr[j];
            for (int i = j - 1; i >= 0; i--) {//dp过程
    ...
    public static int p(int[] arr, int i, int j) {//只用先取者一个表存储数据
        if (i == j) {
            return arr[i];
        }
        if (i + 1 == j) {
    ...
    public static int win4(int[] arr) {
        ...
        for (int i = 0; i < arr.length - 1; i++) {//递归终止条件
            dp[i][i] = arr[i];
